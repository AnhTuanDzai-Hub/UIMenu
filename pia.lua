...
toggle_Image.BackgroundTransparency = 1
...
local toggled = false
local clickThreshold = 5 -- Khoảng cách nhỏ để phân biệt giữa click và drag

local function update(input)
    local delta = input.Position - dragStart
    toggle_Image.Position = UDim2.new(
        startPos.X.Scale,
...
toggle_Image.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        dragging = true
        dragStart = input.Position
        startPos = toggle_Image.Position
        ...
        input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then
                ...
                if moved < clickThreshold then
                    -- Nếu di chuyển rất ít => xem là click => Toggle Body
                    toggled = not toggled
...
toggle_Image.InputChanged:Connect(function(input)
...
toggle_Image.MouseEnter:Connect(function()
    utility:Tween(Toggle, {BackgroundColor3 = Color3.fromRGB(30, 30, 30)}, 0.15)
end)

toggle_Image.MouseLeave:Connect(function()
    utility:Tween(Toggle, {BackgroundColor3 = Color3.fromRGB(60, 60, 60)}, 0.15)
end)

-- MInimize_Button giữ nguyên code:
